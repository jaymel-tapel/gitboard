#!/bin/bash
# Hook script: Notify server when Claude encounters a tool failure
# This is called by Claude Code's PostToolUseFailure hook

PREFIX="[notify-error.sh]"
...
if [ -n "$GITBOARD_TICKET_ID" ] && [ -n "$GITBOARD_SERVER_URL" ]; then
    echo "$PREFIX Sending 'error' status for ticket: $GITBOARD_TICKET_ID to $GITBOARD_SERVER_URL"
    ...
    HTTP_RESPONSE=$(curl -s -w "\nHTTP_STATUS:%{http_code}" -X POST "${GITBOARD_SERVER_URL}/api/session-status-internal" \
        ...
        -d "{\"ticketId\": \"$GITBOARD_TICKET_ID\", \"status\": \"error\"}" 2>&1)
