#!/bin/bash
# Hook script: Notify server when Claude stops/finishes execution
# This is called by Claude Code's Stop hook

PREFIX="[notify-paused.sh]"
...
if [ -n "$GITBOARD_TICKET_ID" ] && [ -n "$GITBOARD_SERVER_URL" ]; then
    echo "$PREFIX Sending 'paused' status for ticket: $GITBOARD_TICKET_ID to $GITBOARD_SERVER_URL"
    ...
    HTTP_RESPONSE=$(curl -s -w "\nHTTP_STATUS:%{http_code}" -X POST "${GITBOARD_SERVER_URL}/api/session-status-internal" \
        ...
        -d "{\"ticketId\": \"$GITBOARD_TICKET_ID\", \"status\": \"paused\"}" 2>&1)
